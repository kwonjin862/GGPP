	...
	public ModelAndView bWrite(BOARD board) throws IllegalStateException, IOException {
		// System.out.println("[2] controller → service \n board : " + board);
				...
				if (boFile != null) {
					...
					// 저장 경로에 파일을 업로드한다.
					String savePath = "D:/spring/GGPP/src/main/webapp/resources/fileUpload/" + fileName;
...
System.out.println("[2] controller → service \n board : " + board);
		...
		if (!bFile.getOriginalFilename().isEmpty()) {
			...
			String deletePath = "D:/spring/GGPP/src/main/webapp/resources/fileUpload/" + board.getBoFileName();
